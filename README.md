...
The CLI emits `generated/go/assembly/invocations_gen.go` with one typed endpoint adapter per selected ordinary canonical Capability, one immutable Kernel catalog, one shared dispatcher, and dependency-ordered application handles. The catalog records the exact contract digest, Plugin ID, provider package, Go Module path and version, generated build identity, and sole-provider or explicit-selection reason. Cross-module adapters copy fields directly and perform explicit conversions only for generated named enum types; they do not use JSON as an in-process type bridge. Raw dispatch applies a 30-second default only when the caller and generated path supply no earlier deadline. Alias IDs never enter the catalog or dispatcher.

Generated application handles are constructed in topological dependency order from the canonical clients required by their lowered contributions. A cycle, missing or repeated dependency, accessor collision, invalid provenance value, or non-ordinary selection reason fails generation. Applications with no ordinary providers still receive a valid empty canonical runtime.
