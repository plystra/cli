...
The application-owned `generated/go/bootstrap` package is the runtime construction and configuration-selection boundary. Its `New` function selects root `plystra.yaml` by default, loads it through the Kernel's bounded regular-file API, validates `timeouts.startup`, resolves Secrets, constructs each selected ordinary provider exactly once, adds the Kernel's intrinsic bindings, publishes the complete canonical invocation runtime, and returns a private redacted `Application`. Passing `--env <environment>` to the generated binary, or setting `PLYSTRA_ENV` when no explicit selector is present, loads root plus exactly one required sparse `plystra.<environment>.yaml` through the same typed field rules used during generation. An explicit flag overrides the ambient selector; unsafe names, missing overlays, unknown fields, invalid types, and prohibited YAML references fail before any Provider constructor runs, while every unselected overlay remains unread. Generate and start the application with the same environment selection. Full-replacement runtime `--config`/`PLYSTRA_CONFIG` selection and compiled application-model compatibility enforcement remain later Gate 10 outcomes.
