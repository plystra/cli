...
Generation also owns `generated/go/assembly/interfaces_gen.go`. Its `NewInterfaceRuntime` creates each selected constructor exactly once in dependency-first order, injects required proxies plus available or unavailable `plystra.Optional[T]` values, shares one concrete instance across every Interface declared by that constructor, creates exact Implementation bindings with constructor, module, selection-reason, and contract-digest provenance, and publishes one complete immutable Kernel catalog before returning typed root Interface accessors. Internal calls remain ordinary typed in-process method calls through the governed proxies. Proxy, adapter, and static-assembly identity and source digests participate in application-model schema version 10; `plystra generate --check` reports a missing or modified file without changing the Project. Generated application-startup integration, lifecycle start and reverse shutdown, and partial-startup cleanup remain later Gate 8 work.
