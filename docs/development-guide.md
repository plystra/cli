...
and visible inactive Interfaces. Use `plystra inspect implementations` to show
...
Project-relative selection when rerunning. Treat the uppercase
`PLYSTRA_<AREA>_<CONDITION>` code
...
`PLYSTRA_GENERATION_API_UNSUPPORTED` points directly to the unsupported
`generation.api` scalar in the owning module-relative `plugin.yaml`. Change the
