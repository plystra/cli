...
func generatedNodeContext(t *testing.T) generation.Context {
	...
	input := generation.Input{
		...
		Capabilities: []generation.CapabilityInput{
			{ContractJSON: json.RawMessage(`{"id":"order.create/v1","request":{"dry_run":{"type":"boolean"},"space_id":{"type":"string","required":true}},"response":{"order_id":{"type":"string","required":true}},"errors":["invalid_state"],"extensions":{"audit":{"event":"order.create"},"authn":{"authenticated":true},"authz":{"permission":"order.create","space":"request.space_id"}}}`), Exposure: generation.Exposure{Go: true, HTTP: true}},
			{ContractJSON: json.RawMessage(`{"id":"authn.session.verify/v1","request":{"token":{"type":"string","required":true}},"response":{"verified":{"type":"object","required":true}},"errors":["invalid_credentials"]}`), Exposure: generation.Exposure{Go: true}},
			{ContractJSON: json.RawMessage(`{"id":"authz.check/v1","request":{"permission":{"type":"string","required":true},"space_id":{"type":"string","required":true}},"response":{"allowed":{"type":"boolean","required":true}},"errors":["decision_failed"]}`), Exposure: generation.Exposure{Go: true}},
			{ContractJSON: json.RawMessage(`{"id":"audit.write/v1","request":{"event":{"type":"string","required":true},"space_id":{"type":"string","required":true}},"response":{},"errors":["write_failed"]}`), Exposure: generation.Exposure{Go: true}},
