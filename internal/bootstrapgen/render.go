...
// document, constructs the static Interface runtime and temporary legacy
// instances, and owns the available lifecycle boundary.
func Render(options Options) ([]byte, error) {
	...
	fmt.Fprintln(&source, "\t// ErrApplicationStart reports a redacted application startup failure.")
	fmt.Fprintln(&source, "\tErrApplicationStart = errors.New(\"generated application startup failed\")")
	fmt.Fprintln(&source, "\t// ErrApplicationStop reports a redacted application shutdown failure.")
	...
	fmt.Fprintln(&source, "}")
	fmt.Fprintln(&source)
	fmt.Fprintln(&source, "// Application is one constructed static Interface runtime and temporary legacy lifecycle boundary.")
	fmt.Fprintln(&source, "// Static Implementation and temporary legacy instances plus resolved runtime configuration remain private.")
	fmt.Fprintln(&source, "type Application struct {")
	fmt.Fprintln(&source, "\tinterfaces     applicationassembly.InterfaceRuntime")
	...
	fmt.Fprintln(&source, "// constructs the frozen Interface runtime and every temporary legacy instance, then binds available lifecycle order.")
	...
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\treturn &Application{interfaces: interfaces, providers: providers, invocations: invocations, lifecycle: manager, startupTimeout: startupTimeout}, nil")
	fmt.Fprintln(&source, "}")
	fmt.Fprintln(&source)
	fmt.Fprintln(&source, "// Valid reports whether static Interface construction and lifecycle binding completed.")
	fmt.Fprintln(&source, "func (a *Application) Valid() bool {")
	fmt.Fprintln(&source, "\treturn a != nil && a.interfaces.Valid() && a.providers.Valid() && a.invocations.Valid() && a.lifecycle != nil && a.startupTimeout > 0")
	fmt.Fprintln(&source, "}")
	fmt.Fprintln(&source)
	fmt.Fprintln(&source, "// Interfaces returns the immutable governed typed Interface runtime.")
	fmt.Fprintln(&source, "func (a *Application) Interfaces() applicationassembly.InterfaceRuntime {")
	fmt.Fprintln(&source, "\tif !a.Valid() {")
	fmt.Fprintln(&source, "\t\treturn applicationassembly.InterfaceRuntime{}")
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\treturn a.interfaces")
	fmt.Fprintln(&source, "}")
	fmt.Fprintln(&source)
	...
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\tstaticState := a.interfaces.State()")
	fmt.Fprintln(&source, "\tlegacyState := a.lifecycle.State()")
	fmt.Fprintln(&source, "\tif staticState == kernellifecycle.StateFailed || legacyState == kernellifecycle.StateFailed {")
	fmt.Fprintln(&source, "\t\treturn kernellifecycle.StateFailed")
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\tif staticState == kernellifecycle.StateStarting || legacyState == kernellifecycle.StateStarting {")
	fmt.Fprintln(&source, "\t\treturn kernellifecycle.StateStarting")
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\tif staticState == kernellifecycle.StateStopping || legacyState == kernellifecycle.StateStopping {")
	fmt.Fprintln(&source, "\t\treturn kernellifecycle.StateStopping")
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\tif staticState == legacyState {")
	fmt.Fprintln(&source, "\t\treturn staticState")
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\tif staticState == kernellifecycle.StateRunning || legacyState == kernellifecycle.StateRunning {")
	fmt.Fprintln(&source, "\t\treturn kernellifecycle.StateStarting")
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\treturn kernellifecycle.StateStopping")
	fmt.Fprintln(&source, "}")
	fmt.Fprintln(&source)
	fmt.Fprintln(&source, "// Start starts static lifecycle-aware Implementations in dependency order within timeouts.startup.")
	...
	fmt.Fprintln(&source, "}")
	fmt.Fprintln(&source)
	fmt.Fprintln(&source, "// Stop shuts down active lifecycles in reverse startup order.")
	...
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\tlegacyError := a.lifecycle.Stop(ctx)")
	fmt.Fprintln(&source, "\tinterfaceError := a.interfaces.Stop(ctx)")
