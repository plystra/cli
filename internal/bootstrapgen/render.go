...
// document, constructs the static Interface runtime and selected legacy
// providers, and owns the available lifecycle boundary.
func Render(options Options) ([]byte, error) {
	...
	fmt.Fprintln(&source, "}")
	fmt.Fprintln(&source)
	fmt.Fprintln(&source, "// Application is one constructed static Interface runtime and selected-provider lifecycle.")
	fmt.Fprintln(&source, "// Implementation and Provider instances plus resolved runtime configuration remain private.")
	...
	fmt.Fprintln(&source, "// constructs the frozen Interface runtime and every selected legacy provider, then binds available lifecycle order.")
	...
	fmt.Fprintln(&source, "\t}")
	fmt.Fprintln(&source, "\tinterfaces, err := applicationassembly.NewInterfaceRuntime(applicationassembly.ConstructorConfiguration{})")
	fmt.Fprintln(&source, "\tif err != nil {")
