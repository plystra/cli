...
func TestResolveSourcesReportsConstraintConflict(t *testing.T) {
	t.Parallel()
	...
	writePlugin(t, root, "profile", "id: acme.app.profile\nprovides: [account.register/v1]\n")
	id := mustCapabilityID(t, "account.register/v1")
	writeCapabilitySource(t, filepath.Join(root, "account"), id, []byte("id: account.register/v1\nrequest:\n  email: {type: string, required: true, constraints: {min_length: 3, max_length: 254}}\n"+querySemanticsYAML))
	writeCapabilitySource(t, filepath.Join(root, "profile"), id, []byte("id: account.register/v1\nrequest:\n  email: {type: string, required: true, constraints: {min_length: 3, max_length: 320}}\n"+querySemanticsYAML))
	...
	differences := conflict.Differences()
	if len(differences) != 1 ||
		differences[0].Path() != "request.email.constraints.max_length" ||
		differences[0].Baseline() != "254" ||
		differences[0].Conflicting() != "320" {
		t.Fatalf("constraint differences = %#v", differences)
	}
	for _, required := range []string{
		"acme.app.account",
		"acme.app.profile",
