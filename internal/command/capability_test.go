...
func TestRunCapabilityCreateAndImplementUsePublicTransactionalSurface(t *testing.T) {
	...
	wantError = "create capability: capability version requires confirmation: records.archive/v3 is skipped without visible version history\n\n" +
		"Recovery:\nReview the visible Capability versions, then rerun the same `plystra capability create` command with `--confirm`.\n\n" +
		"Diagnostic: " + diagnosticcode.CapabilityCreateConfirmationRequired + "\n"
	if exitCode != 1 || stdout != "" || stderr != wantError {
...
func TestRunCapabilityExposeRejectsEventAndStreamAndRollsBack(t *testing.T) {
	kinds := []struct {
		name      string
		semantics string
	}{
...
`,
	...
	selections := []struct {
		...
		{name: "environment", arguments: []string{"--env", "production"}, selectedPath: "plystra.production.yaml", selectedData: "# Production transport choices.\n{}\n"},
		{name: "full replacement", arguments: []string{"--config", "deploy/customer-a.yaml"}, selectedPath: "deploy/customer-a.yaml", selectedData: "# Customer transport choices.\n{}\n"},
	}
	...
	for _, kind := range kinds {
		...
		for _, selection := range selections {
			selection := selection
			t.Run(kind.name+"/"+selection.name, func(t *testing.T) {
				root := writeCapabilityCommandModule(t)
				...
				if exitCode != 1 || stdout != "" {
					t.Fatalf("capability expose %s = exit %d, stdout %q, stderr %q", kind.name, exitCode, stdout, stderr)
				}
				for _, want := range []string{
				...
				if after := commandTree(t, root); !reflect.DeepEqual(after, before) {
					t.Fatalf("failed %s exposure changed Project:\nbefore: %#v\nafter:  %#v", kind.name, before, after)
				}
				if got := string(readCommandFile(t, root, filepath.ToSlash(selection.selectedPath))); got != selection.selectedData {
					t.Fatalf("failed %s exposure did not restore selected configuration: %q", kind.name, got)
				}
				assertNoCommandTransactions(t, root)
			})
		}
