...
func TestParseUseArguments(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "default",
			arguments: []string{"use", "email.send/v1", "example.com/email/smtp.New"},
			want:      useArguments{interfaceID: "email.send/v1", constructor: "example.com/email/smtp.New"},
		},
		{
			name:      "environment",
			arguments: []string{"use", "email.send/v1", "example.com/email/smtp.New", "--env", "production"},
			want:      useArguments{interfaceID: "email.send/v1", constructor: "example.com/email/smtp.New", environment: "production"},
		},
		{
			name:      "configuration",
...
func TestParseUseArgumentsRejectsInvalidForms(t *testing.T) {
	...
	for _, arguments := range [][]string{
		...
		{"use", "email.send/v1", "example.com/email/smtp.New", "extra"},
		{"use", "email.send/v1", "example.com/email/smtp.New", "--env"},
