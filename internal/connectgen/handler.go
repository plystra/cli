...
func renderCanonical(
	...
	fmt.Fprintln(&source, ")")
	fmt.Fprintln(&source)
	fmt.Fprintln(&source, "// RootContext establishes the trusted Kernel root for one external Connect request. The generated handler preserves explicit caller cancellation even when the returned context is detached.")
