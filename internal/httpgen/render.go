...
func render(modulePath string, target CanonicalTargetView, plan *generationlowering.Plan, configurationProvenance transportprovenance.Provenance) (File, error) {
	...
	renderInvocationBoundary(&source, useHTTPPath)
...
func renderServeHTTP(source *strings.Builder, useHTTPPath bool) {
	...
	fmt.Fprintln(source, "\t\treturn")
	fmt.Fprintln(source, "\t}")
	fmt.Fprintln(source, "\tif err := applicationinvocation.ValidateRequest(decoded); err != nil {")
...
func renderInvocationBoundary(source *strings.Builder, useHTTPPath bool) {
	...
	fmt.Fprintln(source, "\t\t}")
	fmt.Fprintln(source, "\t}()")
	fmt.Fprintln(source, "\tinput := applicationinvocation.SafeTransportError(err)")
	fmt.Fprintln(source, "\tif semantic := input.SemanticErrorCode(); semantic != \"\" {")
	fmt.Fprintln(source, "\t\tplystraWriteError(writer, http.StatusUnprocessableEntity, \"capability_error\", semantic)")
	fmt.Fprintln(source, "\t\treturn")
	fmt.Fprintln(source, "\t}")
	fmt.Fprintln(source, "\tif class := input.KernelErrorClass(); class != \"\" {")
	fmt.Fprintln(source, "\t\tplystraWriteError(writer, plystraStatus(class), class, input.KernelDetailCode())")
	fmt.Fprintln(source, "\t\treturn")
	fmt.Fprintln(source, "\t}")
	fmt.Fprintln(source, "\tplystraWriteError(writer, http.StatusInternalServerError, \"internal\", \"\")")
	fmt.Fprintln(source, "}")
	fmt.Fprintln(source)
	fmt.Fprintln(source, "func plystraStatus(code string) int {")
	...
	fmt.Fprintln(source, "\tcase \"conflict\", \"version_incompatible\":")
	fmt.Fprintln(source, "\t\treturn http.StatusConflict")
	fmt.Fprintln(source, "\tcase \"timeout\", \"unavailable\", \"result_unknown\":")
