...
func render(planned plan) ([]byte, error) {
	...
	fmt.Fprintln(&source, "var ErrInterfaceAssembly = errors.New(\"assemble static Interface runtime\")")
	fmt.Fprintln(&source)
	...
	fmt.Fprintf(&source, "\treturn runtime.initialized && runtime.dispatcher != nil && runtime.dispatcher.Published() && len(runtime.catalog.Bindings()) == %d\n", len(kernelintrinsic.InterfaceDefinitions())+len(planned.bindings))
	fmt.Fprintln(&source, "}")
	fmt.Fprintln(&source)
	...
	fmt.Fprintln(&source, "func NewInterfaceRuntime(configuration ConstructorConfiguration) (InterfaceRuntime, error) {")
