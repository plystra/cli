...
func renderREADME(packageName string, operations []renderedOperation, interfaces []renderedInterface) []byte {
	...
	fmt.Fprintln(&readme, "  credentialPolicy: { mode: \"anonymous\" },")
	fmt.Fprintln(&readme, "});")
	fmt.Fprintln(&readme)
	if len(interfaces) != 0 {
