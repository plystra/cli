...
Every property uses the Interface field's effective JSON name. Authored required markers produce required readonly properties; every other field remains optional. Nested same-package messages and the declared semantic-error-code union are exported from the package root together with the canonical request and response types.

## Exposed Interface methods

- `records.echo/v1`: `client.records.echo.v1(request)` or `createRecordsEchoV1(options)(request)` (`sha256:9edac981e07d60dff81938719b99315face2aa89e8111426f76030fc1de6fe4d`)
...
```ts
import { createPlystraClient, type RecordsEchoV1Request } from "@acme/project-sdk";

declare const request: RecordsEchoV1Request;
