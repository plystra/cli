...
func createKernelProxy(t *testing.T) string {
	...
	files := []struct {
		...
		{name: "lifecycle/lifecycle.go", data: []byte("package lifecycle\n\nimport (\n\t\"context\"\n\t\"time\"\n\n\t\"github.com/plystra/kernel/plugin\"\n)\n\ntype Provider interface {\n\tStart(context.Context) error\n\tStop(context.Context) error\n}\n\ntype State string\ntype Binding struct{ provider Provider }\ntype Manager struct{ bindings []Binding; started int }\ntype ManagerOptions struct { RollbackTimeout time.Duration }\n\nfunc NewBinding(_ plugin.ID, provider Provider) (Binding, error) { return Binding{provider: provider}, nil }\nfunc NewManager(_ ManagerOptions, bindings []Binding) (*Manager, error) { return &Manager{bindings: append([]Binding(nil), bindings...)}, nil }\nfunc (manager *Manager) State() State { if manager.started > 0 { return \"started\" }; return \"new\" }\nfunc (manager *Manager) Start(ctx context.Context) error {\n\tfor index := range manager.bindings {\n\t\tif err := manager.bindings[index].provider.Start(ctx); err != nil { return err }\n\t\tmanager.started++\n\t}\n\treturn nil\n}\nfunc (manager *Manager) Stop(ctx context.Context) error {\n\tfor index := manager.started - 1; index >= 0; index-- {\n\t\tif err := manager.bindings[index].provider.Stop(ctx); err != nil { return err }\n\t\tmanager.started--\n\t}\n\treturn nil\n}\n")},
...
func assertPlystraSkill(t *testing.T, root, modulePath string) {
	...
	for _, required := range []string{
		...
		"pre-cancelled direct call and in-flight canonical plus Alias HTTP calls",
		"no response. Cancellation is best-effort",
