...
` + "`PLYSTRA_RESOLVE_UNKNOWN_INTERFACE`" + ` reports every effective ` + "`interfaces.require`" + ` or ` + "`http.expose`" + ` declaration as a ` + "`declaration`" + ` or ` + "`exposure`" + ` source, or every effective ` + "`interfaces.use`" + ` declaration as ` + "`implementation-selection`" + `. Current selected documents and explicitly adopted export contributors retain their owning Project module and module-relative path.
...
` + "`PLYSTRA_CONFIGURATION_INHERITED_CONFLICT`" + ` reports every explicitly adopted export that contributes a competing declaration as a sorted ` + "`configuration-declaration`" + ` source. The problem identifies the exact field while configuration values and Secret-reference targets stay redacted. Apply an explicit replacement or removal in the selected root, environment, or complete-replacement document.
...
` + "`PLYSTRA_CONSTRUCTOR_CONFIGURATION_VALUES_INVALID`" + ` identifies a safe declared field whose value does not match the discovered compiled same-package ` + "`Config`" + ` schema. Correct it in the selected current-Project document or the owning explicitly adopted export. The diagnostic emits exactly one ` + "`configuration-declaration`" + ` source at ` + "`1:1`" + ` and never prints configured values, unknown authored keys, Secret-reference targets, absolute paths, or Module Cache paths.
...
const agentGuidanceReadmeTemplate = `
## AI coding agents
