...
A well-formed exact exposure target absent from the selected visible catalog
uses `PLYSTRA_CAPABILITY_EXPOSE_NOT_VISIBLE` before write planning or mutation;
rerun `plystra capability expose` with one visible exact Capability.
