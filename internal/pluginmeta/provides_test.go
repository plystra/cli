...
func TestAddProvidedPreservesManifestSourceAndAddsSortedCapability(t *testing.T) {
	...
	for _, retained := range []string{"# Account plugin.", "# Stable identity.", "# Profile API.", "# Account API.", "audit.write/v1", "token: {type: secret} # Runtime secret."} {
...
func FuzzAddProvided(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, input string) {
		...
		if err != nil || metadata.ID() != before.ID() || changed == wasPresent || !reflect.DeepEqual(identifierStrings(metadata.Provides()), want) {
			t.Fatalf("Parse(updated) = %#v, %v", metadata, err)
		}
