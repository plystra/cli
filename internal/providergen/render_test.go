...
func TestRenderDerivesHierarchicalCapabilityPath(t *testing.T) {
	...
	got := strings.Join(strings.Fields(string(file.Data())), " ")
	if file.Path() != "generated/go/providers/authn/login/oidc/complete/v1/provider_gen.go" || file.PackageName() != "authnloginoidccompletev1" || !strings.Contains(got, `Complete(ctx context.Context, request contract.Request) (contract.Response, error)`) {
